...
class FCGS_D(nn.Module):
    def __init__(self, args):
        ...
        self.FreqEncoder = tcnn.NetworkWithInputEncoding(n_input_dims=3, n_output_dims=self.lat_dim, encoding_config=self.GridEncoder_config['encoding_freq'], network_config=self.GridEncoder_config['network'])
    ...
    def decompress(self, cur_gaussians_path, y_hat_bit_path, z_hat_bit_path, mask_bit_path, dynamicGS_type='3dgstream'):
        ...
        data_y_hat_motion = y_hat_motion.sum(dim=1, keepdim=True).detach().cpu().numpy()
        data_dec_motion = (dec_motion * motion_weight).sum(dim=1, keepdim=True).detach().cpu().numpy()
    ...
    def forward(self):
        ...
        idx = torch.randint(0, N_gaussian, (N_gaussian//20,), device=cur_xyz.device)
