...
for lambda_size in lambda_size_list:
    model_path = f'./3DGStream-Res/all_scenes-{lambda_size}-joint-stepping-0404-meetroom-gof5-i'
    checkpoint_path = f'{model_path}/.pth'
    script = f"python train_fcgsd.py --gpu 2 --eval  --dynamicGS_type 3dgstream_implicit \
...
for lambda_size in lambda_size_list:
    model_path = f'./3DGStream-Res/all_scenes-{lambda_size}-joint-stepping-0404-meetroom-gof5-i'
    checkpoint_path = f'{model_path}/.pth'
    script = f"python train_fcgsd.py --gpu 2 --eval --use_gof  --dynamicGS_type 3dgstream_implicit \
        --Q_y 1 --Q_z 1 --lr 1e-3 --lambda_size {lambda_size}  --residual_num 1\
        --conduct_compress --conduct_decompress --use_scenes \
        --frame_start 1 --frame_end 30 --test_frame_start 1 --test_frame_end 30 --gof_size 5 \
