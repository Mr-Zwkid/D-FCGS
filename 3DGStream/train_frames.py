...
import re
from cache_warmup import cache_warmup
...
def training_one_frame(dataset, opt, pipe, load_iteration, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # Query the NTC and update the Gaussians
        gaussians.query_ntc()
...
def training_report(tb_writer, iteration, Ll1, Lds, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    if iteration in testing_iterations:
        ...
        return {'last_test_psnr':last_test_psnr.cpu().numpy() if not isinstance(last_test_psnr, float) else last_test_psnr
    ...
    if(args.opt_type=='3DGStream'):
        ...
        if s1_ress !=[]:
            for idx, s1_res in enumerate(s1_ress):
                ...
                res_dict[f'stage1/psnr_{idx}']=float(s1_res['last_test_psnr'])
        ...
        if s2_ress !=[]:
            for idx, s2_res in enumerate(s2_ress):
                ...
                res_dict[f'stage2/psnr_{idx}']=float(s2_res['last_test_psnr'])
...
def train_frames(lp, op, pp, args):
    ...
    for frame in frames:
        ...
        args.output_path = os.path.join(output_path, frame, 'gs')
...
if __name__ == "__main__":
    ...
    json_namespace = json.dumps(serializable_namespace, indent=1)
    os.makedirs(args.output_path, exist_ok = True)
    ...
    # cache warmup
    print("Cache warmup!")
    scene = args.output_path.split('/')[-1]
    if args.ntc_path is None:
        args.ntc_path = os.path.join(args.output_path, f"{scene}_ntc_params_F_4.pth")
    if not os.path.exists(args.ntc_path):
        print(f"Cache file {args.ntc_path} not found, generating...")
        cache_warmup(scene, ['F_4'], args)
